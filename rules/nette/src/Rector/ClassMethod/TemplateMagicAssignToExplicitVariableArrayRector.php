...
final class TemplateMagicAssignToExplicitVariableArrayRector extends AbstractRector
{
    ...
    public function __construct(ActionRenderFactory $actionRenderFactory, TemplatePropertyAssignCollector $templatePropertyAssignCollector, RenderMethodAnalyzer $renderMethodAnalyzer, NetteClassAnalyzer $netteClassAnalyzer)
    {
    ...
    {
        return new RuleDefinition('Change `$this->templates->{magic}` to `$this->template->render(..., $values)` in components', [
...
CODE_SAMPLE
    ...
    private function replaceConditionalAssignsWithVariables(ClassMethod $classMethod, MagicTemplatePropertyCalls $magicTemplatePropertyCalls): void
    {
        $this->traverseNodesWithCallable((array) $classMethod->stmts, function (Node $node) use ($magicTemplatePropertyCalls): ?Assign {
