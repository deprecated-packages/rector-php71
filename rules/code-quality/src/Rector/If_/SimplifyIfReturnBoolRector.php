...
final class SimplifyIfReturnBoolRector extends AbstractRector
{
    ...
    {
        return new RuleDefinition('Shortens if return false/true to direct return', [
...
, 'return strpos($docToken->getContent(), "\n") === false;'),
    ...
    private function processReturnFalse(If_ $if, Return_ $nextReturnNode): ?Return_
    {
        if ($if->cond instanceof Identical) {
            return new Return_($this->boolCastOrNullCompareIfNeeded(new NotIdentical($if->cond->left, $if->cond->right)));
        }
