...
abstract class AbstractPropertyRenamer implements RenamerInterface
{
     ...
     * @required
     */
    public function autowireAbstractPropertyRenamer(CallableNodeTraverser $callableNodeTraverser, NodeNameResolver $nodeNameResolver, NotPrivatePropertyGuard $notPrivatePropertyGuard, RamseyUuidInterfaceGuard $ramseyUuidInterfaceGuard, DateTimeAtNamingConventionGuard $dateTimeAtNamingConventionGuard, PropertyRenameGuard $propertyRenameGuard, HasMagicGetSetGuard $hasMagicGetSetGuard): void
    {
    ...
    private function renamePropertyFetchesInClass(PropertyRename $propertyRename): void
    {
        ...
        $this->callableNodeTraverser->traverseNodesWithCallable($propertyRename->getClassLike(), function (Node $node) use ($propertyRename): ?Node {
            if ($this->nodeNameResolver->isLocalPropertyFetchNamed($node, $propertyRename->getCurrentName()) && $node instanceof PropertyFetch) {
