...
final class AddUuidMirrorForRelationPropertyRector extends AbstractRector
{
    ...
    public function __construct(PhpDocTagNodeFactory $phpDocTagNodeFactory, UuidMigrationDataCollector $uuidMigrationDataCollector, DoctrineDocBlockResolver $doctrineDocBlockResolver, JoinColumnTagValueNodeFactory $joinColumnTagValueNodeFactory)
    {
    ...
    {
        return new RuleDefinition('Adds $uuid property to entities, that already have $id with integer type.' .
...
CODE_SAMPLE
    ...
    private function addNewPropertyToCollector(Property $property, string $oldPropertyName, string $uuidPropertyName): void
    {
        $phpDocInfo = $this->phpDocInfoFactory->createFromNodeOrEmpty($property);
        $doctrineRelationTagValueNode = $phpDocInfo->getByType(DoctrineRelationTagValueNodeInterface::class);
