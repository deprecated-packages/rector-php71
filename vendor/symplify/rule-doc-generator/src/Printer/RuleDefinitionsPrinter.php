...
final class RuleDefinitionsPrinter
{
    ...
    public function __construct(CodeSamplePrinter $codeSamplePrinter, KeywordHighlighter $keywordHighlighter, CategoryResolver $categoryResolver)
    {
    ...
    private function createCategoryMenu(array $ruleDefinitionsByCategory): array
    {
        ...
        $lines[] = '## Categories';
        foreach ($ruleDefinitionsByCategory as $category => $ruleDefinitions) {
            $lines[] = sprintf('- [%s](#%s) (%d)', $category, Strings::webalize($category), count($ruleDefinitions));
        }
        $lines[] = '<br>';
        return $lines;
    }
