...
final class AutoBindNodeFactory
{
    ...
    public function __construct(CommonNodeFactory $commonNodeFactory, ArgsNodeFactory $argsNodeFactory, SymfonyVersionFeatureGuardInterface $symfonyVersionFeatureGuard, TagsServiceOptionKeyYamlToPhpFactory $tagsServiceOptionKeyYamlToPhpFactory)
    {
    ...
    private function createPublicPrivate($value, MethodCall $methodCall, string $type): MethodCall
    {
        ...
        // default value
        if ($type === self::TYPE_DEFAULTS) {
            if ($this->symfonyVersionFeatureGuard->isAtLeastSymfonyVersion(SymfonyVersionFeature::PRIVATE_SERVICES_BY_DEFAULT)) {
                return $methodCall;
            }
