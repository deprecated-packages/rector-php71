...
abstract class AbstractTagValueNode implements AttributeAwareNodeInterface, PhpDocTagValueNode
{
     ...
     */
    public function __construct(ArrayPartPhpDocTagPrinter $arrayPartPhpDocTagPrinter, TagValueNodePrinter $tagValueNodePrinter, array $items = [], ?string $originalContent = null)
    {
    ...
    protected function filterOutMissingItems(array $contentItems): array
    {
        ...
        return ArrayItemStaticHelper::filterAndSortVisibleItems($contentItems, $this->tagValueNodeConfiguration->getOrderedVisibleItems());
    }
    ...
    {
        // @todo decouple
        if ($this->tagValueNodeConfiguration->getOriginalContent() !== null && Strings::endsWith($this->tagValueNodeConfiguration->getOriginalContent(), '()')) {
            return true;
        }
