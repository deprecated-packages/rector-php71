...
final class PhpDocInfoPrinter
{
    ...
    private function printNode(AttributeAwareNodeInterface $attributeAwareNode, ?StartAndEnd $startAndEnd = null, int $key = 0, int $nodeCount = 0): string
    {
        $output = '';
    ...
    {
        $lastTokenPosition = $this->attributeAwarePhpDocNode->getAttribute(Attribute::LAST_TOKEN_POSITION) ?: $this->currentTokenPosition;
     ...
     */
    private function printPhpDocTagNode(PhpDocTagNode $phpDocTagNode, StartAndEnd $startAndEnd, string $output): string
    {
    ...
    private function getRemovedNodesPositions(): array
    {
        ...
        $removedNodes = array_diff($this->phpDocInfo->getOriginalPhpDocNode()
            ->children, $this->attributeAwarePhpDocNode->children);
    ...
    {
        $hasDescriptionWithOriginalSpaces = $attributeAwarePhpDocTagNode->getAttribute(Attribute::HAS_DESCRIPTION_WITH_ORIGINAL_SPACES);
