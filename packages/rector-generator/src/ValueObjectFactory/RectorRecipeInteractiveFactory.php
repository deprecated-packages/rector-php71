...
CODE_SAMPLE;
    ...
    public function __construct(PackageNamesProvider $packageNamesProvider, NodeTypesProvider $nodeTypesProvider, SetsListProvider $setsListProvider, SymfonyStyle $symfonyStyle)
    {
    ...
    {
        $rectorRecipe = new RectorRecipe($this->askForPackageName(), $this->askForRectorName(), $this->askForNodeTypes(), $this->askForRectorDescription(), self::EXAMPLE_CODE_BEFORE, self::EXAMPLE_CODE_AFTER);
    ...
    {
        $choiceQuestion = new ChoiceQuestion(sprintf('For what Nodes should the Rector be run (e.g. <fg=yellow>%s</>)', 'Expr/MethodCall'), $this->nodeTypesProvider->provide());
