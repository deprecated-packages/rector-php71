...
final class AttributeAwareNodeFactoryGenerator extends AbstractAttributeAwareNodeGenerator
{
    ...
    public function __construct(AttributeAwareClassFactoryFactory $attributeAwareClassFactoryFactory, AttributeClassNaming $attributeClassNaming, SymfonyStyle $symfonyStyle)
    {
    ...
    {
        $shortClassName = $this->attributeClassNaming->createAttributeAwareFactoryShortClassName($phpDocParserNodeClass);
    ...
    private function reportGeneratedAttributeAwareNode(string $missingNodeClass, string $filePath): void
    {
        ...
        $message = sprintf('Class "%s" now has freshly generated "%s" in "%s"', $missingNodeClass, $attributeAwareFullyQualifiedClassName, $filePath);
        $this->symfonyStyle->note($message);
    }
}
