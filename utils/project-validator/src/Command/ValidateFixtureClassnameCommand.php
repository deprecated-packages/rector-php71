...
final class ValidateFixtureClassnameCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($fixtureFileInfos as $fixtureFileInfo) {
            ...
            $incorrectClassNameFiles = $this->checkAndFixClassName($fileContent, $fixtureFileInfo, $incorrectClassNameFiles, $optionFix);
        }
        if ($incorrectClassNameFiles !== []) {
            $this->symfonyStyle->listing($incorrectClassNameFiles);

            $message = sprintf('Found %d fixture files with invalid class name which not follow psr-4 defined in composer.json', count($incorrectClassNameFiles));
