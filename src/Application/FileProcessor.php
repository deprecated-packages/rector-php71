...
final class FileProcessor
{
    ...
    public function __construct(AffectedFilesCollector $affectedFilesCollector, CurrentFileInfoProvider $currentFileInfoProvider, FormatPerservingPrinter $formatPerservingPrinter, Lexer $lexer, NodeScopeAndMetadataDecorator $nodeScopeAndMetadataDecorator, Parser $parser, PostFileProcessor $postFileProcessor, RectorNodeTraverser $rectorNodeTraverser, StubLoader $stubLoader, TokensByFilePathStorage $tokensByFilePathStorage)
    {
    ...
    private function makeSureFileIsParsed(SmartFileInfo $smartFileInfo): void
    {
        ...
        throw new ShouldNotHappenException(sprintf('File "%s" was not preparsed, so it cannot be printed.%sCheck "%s" method.', $smartFileInfo->getRealPath(), PHP_EOL, self::class . '::parseFileInfoToLocalCache()'));
    }
}
