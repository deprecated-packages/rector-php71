...
final class NodeFactory
{
    ...
    public function __construct(BuilderFactory $builderFactory, PhpDocInfoFactory $phpDocInfoFactory, PhpVersionProvider $phpVersionProvider, StaticTypeMapper $staticTypeMapper, NodeNameResolver $nodeNameResolver)
    {
    ...
    {
        $classNameNode = in_array($className, self::REFERENCES, true) ? new Name($className) : new FullyQualified($className);
    ...
    {
        return new StaticCall(new Name(self::REFERENCE_PARENT), new Identifier(MethodName::CONSTRUCT), $this->convertParamNodesToArgNodes($params));
    }
    ...
    private function createArrayItem($item, $key = null): ArrayItem
    {
        ...
        throw new NotImplementedException(sprintf('Not implemented yet. Go to "%s()" and add check for "%s" node.', __METHOD__, is_object($item) ? get_class($item) : $item));
    }
