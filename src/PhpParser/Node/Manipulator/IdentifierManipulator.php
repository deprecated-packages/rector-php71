...
final class IdentifierManipulator
{
    ...
    private function ensureNodeHasIdentifier(Node $node): void
    {
        ...
        throw new NodeMissingIdentifierException(sprintf('Node "%s" does not contain a "$name" property with "%s". Pass only one of "%s".', get_class($node), Identifier::class, implode('", "', self::NODE_CLASSES_WITH_IDENTIFIER)));
    }
